...
import { markHomeDeliveredAction } from "@/app/(staff)/vendedor/envios/actions";

type Props = {
  orderId: string;
};

export function HomeDeliverButton({ orderId }: Props) {
  ...
  function handleClick() {
    ...
    startTransition(async () => {
      const result = await markHomeDeliveredAction(orderId);
      if (!result.ok) {
